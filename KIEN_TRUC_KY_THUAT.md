...
Cập nhật source 2026-09-17: worker protocol 2 / version 1.1.0 hỗ trợ CPU hoặc CUDA NVIDIA tùy chọn. Job strategy 4 snapshot `AUTO`/`CPU_ONLY`, tách khỏi fingerprint nội dung; chuyển backend không làm mất cue đã commit. CUDA chọn UUID/VRAM, offload 12/24/36 layer và probe Anh/Trung riêng. Auto kiểm GPU khi bắt đầu dịch, không cần nút kiểm tra riêng; không dùng được GPU thì báo modal theo đúng job và tiếp tục CPU. Lỗi tài nguyên giảm layer một lần rồi giữ CPU trong checkpoint; gói CUDA sai checksum bị từ chối sử dụng, CPU chỉ chạy với model/probe CPU hợp lệ; sai model/protocol vẫn dừng. CPU READY vẫn đủ cho Setup khi chưa cài GPU. Xem [triển khai, bằng chứng và phạm vi chưa nghiệm thu](TRIEN_KHAI_TANG_TOC_DICH_CPU_GPU.md).
