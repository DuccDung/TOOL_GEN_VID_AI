...
const output = path.resolve('.tmp/project-navigation-verification');
...
function html(mode, zoom) {
  ...
  async function inspect(){
    try {
      ...
      const result={mode:${JSON.stringify(mode)},zoom:${zoom},button:{x:rect.x,y:rect.y,width:rect.width,height:rect.height},label:button.textContent.trim()};
...
try {
  for(const [mode,size,zoom] of [['desktop','1600,1000',1],['narrow','720,900',1.25],['create','1200,900',1]]) {
    ...
    const browser=spawn(path.join(process.env['ProgramFiles(x86)'],'Microsoft/Edge/Application/msedge.exe'),
      ['--headless','--disable-gpu','--no-first-run','--no-default-browser-check','--disable-extensions','--disable-background-networking',
       ...
       'http://127.0.0.1:'+server.address().port+'/?mode='+mode+'&zoom='+zoom],{windowsHide:true,stdio:'ignore'});
