...
IF CAST(SERVERPROPERTY('ServerName') AS nvarchar(128)) <> N'DUNGDEV' OR DB_NAME() <> N'VideoFactory'
 THROW 51180, 'Unexpected migration target.', 1;
IF EXISTS (SELECT 1 FROM ai.BudgetReservations WHERE Status=N'Reserved')
 THROW 51181, 'Active reservations detected; defer migration.', 1;
IF OBJECT_ID(N'vs.CloudTranslationJobs',N'U') IS NOT NULL OR EXISTS (SELECT 1 FROM ai.SchemaVersions WHERE Version=N'4.1.6')
 THROW 51182, 'Target changed since inspection.', 1;
GO
