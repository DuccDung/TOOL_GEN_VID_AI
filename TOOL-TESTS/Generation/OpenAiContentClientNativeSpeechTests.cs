...
public sealed class OpenAiContentClientNativeSpeechTests
{
    ...
    public async Task GenerateAsync_RequestsStructuredNativeSpeechAndMapsSpeakerIntent()
    {
        ...
        Assert.Contains("1 to 8 whitespace-separated words", root.GetProperty("input").GetString(), StringComparison.OrdinalIgnoreCase);
    ...
    [Fact]
    public async Task GenerateAsync_RejectsSpokenTextThatCannotFitSceneDuration()
    {
        ...
        Assert.Equal("openai_spoken_text_too_long", exception.Code);
        Assert.Equal("1", Assert.Single(exception.Errors!["sceneNumber"]));
        Assert.Equal("5", Assert.Single(exception.Errors["durationSeconds"]));
        Assert.Equal("9", Assert.Single(exception.Errors["wordCount"]));
        Assert.Equal("8", Assert.Single(exception.Errors["maximumWords"]));
    }

    [Fact]
    public async Task GenerateAsync_SendsEachSceneItsOwnSpeechContract()
    {
        ...
        Assert.Equal(2, input!.Split("1 to 18 whitespace-separated words", StringSplitOptions.None).Length - 1);
    }

    private static string CreateResponse(string spokenText)
    {
        var output = JsonSerializer.Serialize(new
        {
            ...
            scenes = new[]
            {
                new
                {
                    ...
                    visual_prompt = "Minh faces the camera in a bright room and gestures naturally.",
                    character_keys = new[] { "minh" },
                    speech_mode = KlingSpeechModes.OnCameraDialogue,
                    spoken_text = spokenText,
                    speaker_character_key = "minh",
