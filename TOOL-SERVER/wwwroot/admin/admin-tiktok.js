(() => {
  ...
  function renderCredentials(view) {
    ...
    byId('tiktokCredentialsRegion').innerHTML = `<div class="tt-card-heading"><div><span class="tt-eyebrow">ỨNG DỤNG TIKTOK</span><h3 id="tiktokCredentialsTitle">Thông tin kết nối</h3></div>${icon('key')}</div>
      ...
      ${!view.canRotate ? `<div class="tt-inline-warning"><strong>Chưa thể xác minh bản thay thế</strong><p>Còn ${esc(current.connectedUserCount)} tài khoản kết nối và ${esc(current.pendingPublishJobCount)} bài đang xử lý.</p><button type="button" class="tt-text-button" data-tt-action="guide">Xem cách chuẩn bị thay cấu hình</button></div>` : ''}
