...
Script bỏ `appsettings.Development.json` và để trống các giá trị connection string/secret trong `appsettings.json` **của gói**. File source không bị sửa. Hosting phải cấp `ConnectionStrings__VideoFactory`, `Jwt__SigningKey` và các secret cần dùng qua secret manager hoặc biến môi trường của application pool; không ghi chúng vào ZIP hoặc `web.config`.

**Trước request đầu tiên**, xác minh hosting đã cấp `ConnectionStrings__VideoFactory` và `Jwt__SigningKey` (ít nhất 32 byte UTF-8) cho **tiến trình ứng dụng của IIS**, không chỉ cho tài khoản đăng nhập/terminal. Hai giá trị này trong ZIP đều trống. `Program.cs` chạy bootstrap role/license/catalog trên database trước `app.Run()`, nên thiếu connection string hoặc database/schema/quyền chưa sẵn sàng sẽ khiến ANCM trả HTTP 502.5. Không dùng kết quả build hoặc runtime `--list-runtimes` làm bằng chứng startup đã đạt.
