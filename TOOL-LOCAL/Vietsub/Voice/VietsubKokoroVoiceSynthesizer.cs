...
internal sealed record VietsubKokoroWorkerEvent(
    int ProtocolVersion,
    string RequestId,
    string Type,
    int? Index = null,
...
internal sealed class VietsubKokoroVoiceSynthesizer(VietsubKokoroRuntimePaths paths)
    : IVietsubVoiceSynthesizer
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var request = new
        {
            ...
            voicepackPath = paths.VoicePackPath,
            volume = 1.0,
            lengthScale = 1.0,
        ...
        try
        {
            ...
            if (!process.Start())
            {
                throw new VietsubVoiceException(VietsubVoiceErrorCodes.WorkerFailed, "Không thể khởi động Kokoro worker.", true);
            }
            ...
            catch (ArgumentException exception)
            {
                throw new VietsubVoiceException(
                    ...
                    "Danh sách phrase gửi Kokoro có index trùng.",
            ...
            try
            {
                while (true)
                {
                    ...
                    VietsubKokoroWorkerEvent workerEvent;
                    try
                    {
                        workerEvent = JsonSerializer.Deserialize<VietsubKokoroWorkerEvent>(line, JsonOptions)
                            ?? throw new JsonException();
                    }
                    catch (JsonException exception)
                    {
                        throw new VietsubVoiceException(VietsubVoiceErrorCodes.WorkerProtocolInvalid, "Kokoro worker trả về event không hợp lệ.", innerException: exception);
                    }
                ...
                {
                    throw new VietsubVoiceException(VietsubVoiceErrorCodes.WorkerFailed,
                        "Kokoro worker thất bại. Hãy kiểm tra runtime và thử lại.", true);
                }
            ...
            {
                Kill(process);
                throw new VietsubVoiceException(VietsubVoiceErrorCodes.WorkerTimeout, "Kokoro worker không phản hồi trong thời gian cho phép.", true);
            }
    ...
    private static VietsubVoiceException ProtocolError() =>
        new(VietsubVoiceErrorCodes.WorkerProtocolInvalid, "Kokoro worker trả về protocol thiếu, thừa hoặc sai thứ tự.");
