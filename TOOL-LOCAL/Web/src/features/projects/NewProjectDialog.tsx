...
export function NewProjectDialog({ organizationName, busy, error, speechSynchronizationEnabled, shortVideoCharacterOutfitEnabled = false, initialShortMode, voiceOptions, onClose, onCreateLong, onCreateShort }: {
  organizationName: string;
  busy: boolean;
  error: string | null;
  speechSynchronizationEnabled: boolean;
  shortVideoCharacterOutfitEnabled?: boolean;
  initialShortMode?: 'CharacterOutfit';
  ...
  const startWithOutfit = initialShortMode === 'CharacterOutfit' && shortVideoCharacterOutfitEnabled;
  const [kind, setKind] = useState<VideoProjectKind | null>(startWithOutfit ? 'short' : null);
  const [content, setContent] = useState('');
  const [aspectRatio, setAspectRatio] = useState<CreateShortVideoPayload['aspectRatio']>('9:16');
  const [duration, setDuration] = useState(8);
  const [audioEnabled, setAudioEnabled] = useState(true);
  const [shortMode, setShortMode] = useState<'TextOnly' | 'CharacterOutfit'>(startWithOutfit ? 'CharacterOutfit' : 'TextOnly');
  ...
  const valid = Boolean(kind && content.trim() && content.trim().length <= (kind === 'long' ? 300 : 2000) &&
    (kind !== 'short' || shortMode !== 'CharacterOutfit' || shortVideoCharacterOutfitEnabled) &&
  ...
  return <dialog ref={dialog} className="new-project-dialog" aria-labelledby="new-project-title" aria-describedby="new-project-description"
    ...
    {!kind ? <div className="new-project-types">
      <button type="button" disabled={busy} onClick={() => { setKind('short'); setAspectRatio('9:16'); }}>
        ...
        <span>Một cảnh Veo 4, 6 hoặc 8 giây, từ ảnh đầu vào đã duyệt.</span><small>Chọn video ngắn <ArrowRight size={16} /></small>
      </button>
      ...
      if (kind === 'short') onCreateShort({ content: content.trim(), aspectRatio, durationSeconds: duration, audioEnabled,
        ...(shortMode === 'CharacterOutfit' ? { mode: shortMode } : {}) });
      ...
      <fieldset disabled={busy}>
        {kind === 'short' && shortVideoCharacterOutfitEnabled && <label className="new-project-field">Cách tạo video<select value={shortMode} onChange={event => setShortMode(event.target.value as typeof shortMode)}>
          <option value="TextOnly">Từ nội dung mô tả</option><option value="CharacterOutfit">Nhân vật mặc trang phục có sẵn</option>
        </select>{shortMode === 'CharacterOutfit' && <small>Chọn hai ảnh sau khi tạo dự án, duyệt ảnh mặc thử rồi tạo video.</small>}</label>}
        <label className="new-project-field">{kind === 'short' ? shortMode === 'CharacterOutfit' ? 'Tên hoặc mô tả video' : 'Nội dung cảnh' : 'Chủ đề video'}
          <textarea required maxLength={kind === 'short' ? 2000 : 300} value={content} onChange={event => setContent(event.target.value)}
            placeholder={kind === 'short' ? shortMode === 'CharacterOutfit' ? 'Ví dụ: Giới thiệu bộ trang phục mùa hè…' : 'Mô tả chủ thể, bối cảnh, hành động và phong cách của cảnh…' : 'Ví dụ: Lợi ích của việc đạp xe mỗi ngày…'} />
        </label>
        <div className="new-project-options">
          <label className="new-project-field">Tỷ lệ khung hình<select value={aspectRatio} onChange={event => setAspectRatio(event.target.value as typeof aspectRatio)}>
            <option value="9:16">Dọc · 9:16</option><option value="16:9">Ngang · 16:9</option>{kind === 'long' && <option value="1:1">Vuông · 1:1</option>}
          </select></label>
          {kind === 'short' ? <>
            <label className="new-project-field">Thời lượng<select value={duration} onChange={event => setDuration(Number(event.target.value))}>
              {[4, 6, 8].map(seconds => <option key={seconds} value={seconds}>{seconds} giây</option>)}
            </select></label>
